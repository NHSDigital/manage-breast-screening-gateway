...
| Modality C-ECHO / worklist query times out | Missing inbound Windows Firewall rules on the VM. Check the `firewall_rules` health check or `Get-NetFirewallRule -DisplayName "Rubie Gateway*"`; create the rules per the [onboarding runbook troubleshooting](./onboard-hospital-vm.md#modality-c-echo-dicom-ping-to-the-gateway-times-out) |
